...
const PROVIDER_TOKEN_CEILING = {
  ...
  openai: 100000,
  deepseek: 50000,
  omniroute: 24000, // self-hosted gateway - the real limit depends on the model it routes to; tune here
  default: 6000
};
...
const App = {
  ...
  async init() {
    await Settings.init();
    // A ?share=<token> link opens the read-only live viewer, not the workspace
    const shareToken = new URLSearchParams(window.location.search).get('share');
    if (shareToken) { await Viewer.start(shareToken); return; }
    await Logger.restore();
    await this.migrateNicknames();
    ...
    Sync.init(); // non-blocking: pairing/sync only does anything once you've set it up
  ...
  // Stored nickname maps predate newer providers (e.g. OmniRoute), which would
  // make "@omniroute" unresolvable. Fill in anything missing, once.
  async migrateNicknames() {
    const stored = await DB.getSetting('nicknames', null);
    if (!stored) return;
    let changed = false;
    for (const [id, nick] of Object.entries(DEFAULT_NICKNAMES)) {
      if (!(id in stored)) { stored[id] = nick; changed = true; }
    }
    if (changed) await DB.setSetting('nicknames', stored);
  },
  ...
  async openAiSelectMenu() {
    ...
    autoRow.addEventListener('click', () => { this.setTarget(null); dropdown.remove(); });
    ...
    Object.keys(providerDefs).forEach((id, i) => {
      ...
      row.addEventListener('click', () => { this.setTarget(id); dropdown.remove(); });
      dropdown.appendChild(row);
    });
  ...
  // Who answers in THIS session: null = Auto. Remembered per session (and
  // synced with it), so reopening the app shows the same target you left.
  async setTarget(providerId) {
    this.manualTarget = providerId || null;
    if (this.currentSessionId) await DB.setSetting(`providerTarget:${this.currentSessionId}`, this.manualTarget);
    await this.refreshHeaderChips();
  },
  ...
  wireGlobalKeys() {
    document.addEventListener('keydown', async (e) => {
      ...
      if (combo === this.keybinds.toggleAuto) { e.preventDefault(); this.setTarget(null); Toast.show('Back to Auto'); return; }
  ...
  async openSessionMenu(sessionId, anchorEl) {
    ...
    menu.innerHTML = `
      ...
      <div class="palette-item" data-act="accent">${svgIcon('palette')} <span>Set accent color</span></div>
      <div class="palette-item" data-act="export">${svgIcon('doc')} <span>Export as study notes</span></div>
      <div class="palette-item" data-act="share">${svgIcon('share')} <span>Share live link (read-only)</span></div>
    ...
    menu.querySelector('[data-act="accent"]').addEventListener('click', async () => {
      menu.remove();
      await this.openSessionAccentPicker(sessionId, anchorEl);
    });
  ...
  // Creates (or re-copies) a LIVE share link. The link stays valid as you keep
  // chatting: new messages are pushed to it automatically (see Sync.pushShares),
  // and viewers see them without you re-sharing.
  async shareSession(sessionId) {
    try {
      const sessions = await DB.listSessions();
      const s = sessions.find(x => x.id === sessionId);
      const links = await DB.getSetting('shareLinks', []);
      let entry = links.find(l => l.sessionId === sessionId);
      if (!entry) {
        ...
        entry = { token: result.token, sessionId, title: s ? s.title : 'Shared session', createdAt: Date.now() };
        links.push(entry);
        await DB.setSetting('shareLinks', links);
      }
      const link = `${window.location.origin}${window.location.pathname}?share=${encodeURIComponent(entry.token)}`;
      try {
        await navigator.clipboard.writeText(link);
        Toast.show('Live link copied - viewers see new messages as you chat. Revoke it any time in Settings.');
      } catch (e) {
  ...
  async openSession(id) {
    ...
    const sessionAccent = await DB.getSetting(`sessionAccent:${id}`, null);
    if (sessionAccent) main.style.setProperty('--session-accent', sessionAccent);
    else main.style.removeProperty('--session-accent');

    // restore this session's remembered AI target (unknown/removed -> Auto)
    const savedTarget = await DB.getSetting(`providerTarget:${id}`, null);
    const known = await DB.getSetting('nicknames', DEFAULT_NICKNAMES);
    this.manualTarget = (savedTarget && (savedTarget in known)) ? savedTarget : null;
    await this.refreshHeaderChips();
  },

  // Small swatch picker (reuses the same palette as global Appearance) for
  // giving one specific session its own accent color, so busy sessions are
  // visually distinct from each other at a glance. "Default" clears it.
  async openSessionAccentPicker(sessionId, anchorEl) {
    const menu = document.createElement('div');
    menu.className = 'floating-menu';
    menu.style.padding = '8px 10px';
    menu.innerHTML = `
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;max-width:180px">
        ${PALETTE_PRESETS.map(p => `<div class="palette-swatch" data-accent="${p.accent}" style="background:${p.accent}" title="${p.name}"></div>`).join('')}
        <div class="palette-swatch" data-accent="" title="Default" style="background:var(--bg-2);border:1px dashed var(--line-strong)"></div>
      </div>`;
    document.body.appendChild(menu);
    this.positionFloatingMenu(menu, anchorEl);
    menu.querySelectorAll('[data-accent]').forEach(sw => {
      sw.addEventListener('click', async () => {
        const accent = sw.dataset.accent;
        if (accent) await DB.setSetting(`sessionAccent:${sessionId}`, accent);
        else { const kvStore = await tx('kv', 'readwrite'); kvStore.delete(`sessionAccent:${sessionId}`); }
        if (sessionId === this.currentSessionId) {
          const main = document.getElementById('main');
          if (accent) main.style.setProperty('--session-accent', accent);
          else main.style.removeProperty('--session-accent');
        }
        menu.remove();
      });
  ...
  async renderEmptyState() {
    ...
    const greeting = customGreeting || greetings[Math.floor(Math.random() * greetings.length)];
    const starters = [
    ...
    wrap.dataset.custom = customGreeting ? '1' : '0';
    wrap.innerHTML = `
    ...
    if (m.role === 'user') {
      ...
      if (m.attachmentMeta && m.attachmentMeta.length) {
        ...
        m.attachmentMeta.forEach(att => {
          ...
          chip.innerHTML = `${svgIcon(FileHandler.isImageType(att.type) ? 'image' : 'doc')} <span>${this.escapeHtml(att.name)}</span>`;
      ...
      if (m.model) wrap.querySelector('.msg-label').title = 'Model: ' + m.model;
  ...
  async verifyWith(providerId, originalMsg) {
    ...
    try {
      const result = await Providers.chat(providerId, prompt, undefined, { long: true });
  ...
  async regenerate(originalMsg) {
    ...
    let finalText, isError = false;
    try {
      const result = await Providers.chat(providerId, sharedMessages, undefined, { long: true });
  ...
  async compileContext(excludeProviderId, forceProviderId) {
    const nicknames = await DB.getSetting('nicknames', DEFAULT_NICKNAMES);
    const history = await DB.listMessages(this.currentSessionId);
    const visible = history.filter(m => !m.isError && m.role !== 'system');
    ...
    const compilerId = forceProviderId || await this.pickCompilerProvider(excludeProviderId, estimate);
    if (!compilerId) {
  ...
  // Explicit "/compile [ai]": condenses older turns into a recap with the same
  // pipeline as the automatic limit flow (chunking, review-before-apply), and
  // deletes nothing you can see or export.
  async runCompileCommand(arg) {
    const nicknames = await DB.getSetting('nicknames', DEFAULT_NICKNAMES);
    let forced = null;
    if (arg) {
      forced = Router.findMentionedProvider('@' + arg, nicknames);
      if (!forced) { Toast.show(`No AI called "${arg}". Try /compile, or /compile gemini.`, true); return; }
    }
    Logger.info('compile', `manual /compile${forced ? ' with ' + forced : ''}`);
    const ok = await this.compileContext(null, forced);
    if (ok) await this.refreshUsageBar();
  },

  async handleSend(text) {
    const nicknames = await DB.getSetting('nicknames', DEFAULT_NICKNAMES);

    // "/compile" is a command, never chat text - it isn't sent to any AI
    const compileCmd = /^\/compile(?:\s+@?(\S+))?\s*$/i.exec(text);
    if (compileCmd && this.pendingAttachments.length === 0) {
      await this.runCompileCommand(compileCmd[1]);
      return;
    }
    ...
    if (isManualMention) {
      await this.setTarget(targets[0]);
    }
    ...
    // Research mode and Test-me mode legitimately produce longer answers
    // that can take a free-tier model past a quick-chat timeout - mark
    // those (and any unusually long outgoing prompt) as long-running so
    // the Worker gives them real time instead of cutting them off early.
    const effectiveChatMode = modeCmd || await DB.getSetting('chatMode', 'normal');
    const isLongRequest = ['research', 'test', 'outline', 'compare'].includes(effectiveChatMode) || composedText.length > 4000;
    ...
    for (const providerId of targets) {
      ...
      const thinkingEl = this.renderMessage(msgObj);
      let finalText, isError = false, usedModel = null;
      try {
        const result = await Providers.chat(providerId, sharedMessages, undefined, { long: isLongRequest });
        if (result.error) { isError = true; finalText = `(${nickname} error: ${result.message})`; }
        else { finalText = this.stripNamePrefix(result.text); usedModel = result.model || null; }
      } catch (err) {
        isError = true;
        finalText = `(${nickname} error: ${err.message || 'Something went wrong - try again.'})`;
      }
      thinkingEl.querySelector('.bubble').innerHTML = this.renderMarkdown(finalText);
      const saved = await DB.addMessage(this.currentSessionId, { role: 'ai', nickname, content: finalText, isError, model: usedModel });
      Logger.log(isError ? 'error' : 'info', 'provider', `${nickname} ${isError ? 'failed' : 'replied'}${usedModel ? ' (' + usedModel + ')' : ''}`, isError ? finalText.slice(0, 200) : undefined);
      const labelEl = thinkingEl.querySelector('.msg-label');
      if (labelEl && usedModel) labelEl.title = 'Model: ' + usedModel;

...
const App = {
  ...
  // for Cloudflare AI.
  async pickCompilerProvider(excludeId) {
    ...
    for (const id of COMPILE_PROVIDER_PRIORITY) {
      ...
      if (providerDefs[id].kind === 'cloudflare-ai') return id;
      const info = keyInfo[id];
      if (info && info.keys && info.keys.some(k => !k.resting)) return id;
    }
    return null;
  },
  ...
  async branchSession(sessionId) {
    const compilerId = await this.pickCompilerProvider(null);
    if (!compilerId) {
    ...
    const compilePrompt = [
      { role: 'system', content: 'You compress conversation history for reuse as context. Preserve every fact, name, number, date, decision, and specific detail - never summarize away anything concrete. Only strip conversational filler, repeated pleasantries, and back-and-forth phrasing. Output dense prose or tight bullet points, no preamble.' },
      { role: 'user', content: `Compile this entire conversation into a complete, lossless-but-compact recap, to seed a new branch session that continues from it:\n\n${flattened}` }
    ];
    Toast.show(`Branching - compiling shared context with ${nicknames[compilerId] || compilerId}...`);
    let recap;
    try {
      const result = await Providers.chat(compilerId, compilePrompt);
      if (result.error) { Toast.show('Branch failed: ' + result.message, true); return; }
      recap = result.text;
    } catch (e) {
    ...
    await DB.setSetting(`compiledContext:${branch.id}`, { uptoMessageId: null, recap, compiledAt: Date.now(), compiledBy: compilerId, isBranchSeed: true });
    await this.renderSessionList(await DB.listSessions());
  ...
  async compileContext(excludeProviderId) {
    const compilerId = await this.pickCompilerProvider(excludeProviderId);
    if (!compilerId) {
    ...
    const compilePrompt = [
      ...
      { role: 'user', content: `Compile this conversation history into a complete, lossless-but-compact recap:\n\n${flattened}` }
    ];
    Toast.show(`Compiling with ${nicknames[compilerId] || compilerId}...`);
    let recap;
    try {
      const result = await Providers.chat(compilerId, compilePrompt);
      if (result.error) { Toast.show('Compile failed: ' + result.message, true); return false; }
      recap = result.text;
    } catch (e) {
    ...
    await DB.setSetting(`compiledContext:${this.currentSessionId}`, { uptoMessageId: lastKeptId, recap, compiledAt: Date.now(), compiledBy: compilerId });
  ...
  async refreshUsageBar() {
    ...
    const ceilingNote = this.manualTarget ? ` (${(PROVIDER_TOKEN_CEILING[this.manualTarget] || PROVIDER_TOKEN_CEILING.default).toLocaleString()} limit for ${this.manualTarget})` : '';
    bar.textContent = `~${roughTokens.toLocaleString()} tokens in this session (rough estimate)${ceilingNote}`;

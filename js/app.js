const App = {
  ...
  wireHeaderChip() {
    ...
    chipsWrap.addEventListener('click', async (e) => {
      ...
      `;
      const rowStyle = 'padding:7px 10px;border-radius:7px;font-size:13px;cursor:pointer;';
      ...
      Object.keys(providerDefs).forEach(id => {
        ...
        row.textContent = nicknames[id] || providerDefs[id].label;
      ...
      setTimeout(() => {
        document.addEventListener('click', function closeOnce(ev) {
          if (!dropdown.contains(ev.target) && ev.target !== chipsWrap) {
            dropdown.remove();

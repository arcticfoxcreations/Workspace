...
const App = {
  ...
  async refreshHeaderChips() {
    ...
    chipsWrap.innerHTML = `
      <button class="chip chip-active" data-role="target"><span style="margin-right:5px">${glyph}</span>${label}</button>
  ...
  async buildSharedContext() {
    ...
    sharedMessages.unshift({
      role: 'system',
      content: 'You are one voice in a multi-AI workspace. Prior replies from other AIs are shown to you as "[Name]: message" purely as a label so you know who said what - that bracket format is metadata, never something to copy. Never start your own reply with a bracketed name. ' + modeInfo.prompt
    });

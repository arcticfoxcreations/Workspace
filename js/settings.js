...
const FONT_PRESETS = [
  ...
  { name: 'Compact', stack: '"Segoe UI", Arial Narrow, Helvetica, sans-serif' },
  { name: 'Easy-read', stack: 'Verdana, Tahoma, "Segoe UI", sans-serif' }
];
...
const Settings = {
  ...
  scrollToProfile() {
    const el = document.getElementById('profileGroup');
    if (el) { el.scrollIntoView({ behavior: 'smooth', block: 'start' }); }
    setTimeout(() => { const n = document.getElementById('profileNameInput'); if (n) n.focus(); }, 350);
  },
  ...
  async render() {
    ...
    // ---- Profile ----
    html += `
      <div class="settings-group" id="profileGroup">
        <div class="settings-group-title">Profile</div>
        <div style="font-size:11px;color:var(--text-muted);margin-bottom:8px">
          Shared quietly with every AI you talk to in this workspace, so you never have to re-introduce yourself.
        </div>
        <div class="settings-row">
          <label>Name</label>
          <input type="text" id="profileNameInput" placeholder="what should AIs call you?" value="${profile.name || ''}" />
        </div>
        <div class="settings-row">
          <label>Profession</label>
          <input type="text" id="profileProfessionInput" placeholder="e.g. college student, CS major" value="${profile.profession || ''}" />
        </div>
        <div class="settings-row" style="align-items:flex-start">
          <label style="padding-top:6px">About</label>
          <textarea id="profileAboutInput" placeholder="interests, current focus, how you like answers - anything worth an AI knowing" style="flex:1;min-height:60px;background:var(--bg-2);border:0.5px solid var(--line-strong);border-radius:8px;padding:8px 9px;font-size:13px;resize:vertical;font-family:inherit">${profile.about || ''}</textarea>
        </div>
        <button class="small-btn" id="saveProfileBtn">save profile</button>
      </div>`;
    ...
    html += `
      <div class="settings-group" id="appearanceGroup">
        ...
        <div class="settings-row" style="flex-wrap:wrap">
          ...
          <input type="text" id="fontSizeInput" value="${fontSize}" style="max-width:50px" />
        ...
        <div class="settings-row" style="align-items:flex-start">
          <label style="padding-top:5px">Background</label>
          <div style="flex:1;display:flex;flex-direction:column;gap:6px">
            ...
            <div style="display:flex;align-items:center;gap:8px">
              <span style="font-size:11px;color:var(--text-muted);min-width:50px">dim</span>
              <input type="range" id="bgDimInput" min="0" max="0.8" step="0.05" value="${bgDim}" style="flex:1" />
            </div>` : `<div style="font-size:11px;color:var(--text-muted)">None set - the workspace stays plain and distraction-free by default.</div>`}
          </div>
        </div>
      </div>`;

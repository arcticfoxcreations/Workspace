...
const FONT_PRESETS = [
  ...
  { name: 'Rounded', stack: '"Quicksand", -apple-system, sans-serif' },
  { name: 'Compact', stack: '"Roboto Condensed", -apple-system, sans-serif' },
  { name: 'Easy-read', stack: '"Atkinson Hyperlegible", Verdana, sans-serif' },
  { name: 'Mono', stack: '"JetBrains Mono", "SF Mono", Consolas, monospace' }
];
...
const Settings = {
  ...
  async applyStoredAppearance() {
    ...
    const sidebarBlur = await DB.getSetting('sidebarBlur', 0);
    const sidebarOpacity = await DB.getSetting('sidebarOpacity', 1);
    const bubbleShape = await DB.getSetting('bubbleShape', 'rounded');
    const bubbleFill = await DB.getSetting('bubbleFill', 'solid');
    const ultraCompact = await DB.getSetting('ultraCompact', false);
    ...
    appEl.classList.toggle('density-compact', density === 'compact');
    appEl.classList.toggle('density-ultra-compact', !!ultraCompact);
    appEl.classList.remove('bubble-shape-sharp', 'bubble-shape-rounded', 'bubble-shape-pill');
    appEl.classList.add(`bubble-shape-${bubbleShape}`);
    appEl.classList.remove('bubble-fill-solid', 'bubble-fill-outline', 'bubble-fill-glass');
    appEl.classList.add(`bubble-fill-${bubbleFill}`);
    if (bgImage) {
  ...
  async _renderInner() {
    ...
    const bubbleFill = await DB.getSetting('bubbleFill', 'solid');
    const ultraCompact = await DB.getSetting('ultraCompact', false);
    const customGreeting = await DB.getSetting('customGreeting', '');
    ...
    html += `
      <div class="settings-group">
        ...
        <div class="settings-row">
          ...
          <input type="password" id="pinInput" placeholder="PIN or password - letters, numbers, symbols all work" autocapitalize="off" autocorrect="off" spellcheck="false" />
        ...
        <div style="margin-top:14px;padding-top:12px;border-top:0.5px dashed var(--line)">
          ...
          <div style="font-size:11px;color:var(--text-muted);margin-bottom:8px">
            Opens the workspace already unlocked, no PIN typing. Prefer <b>Sync &amp; backup → pairing code</b> below: it gives each device its own revocable token instead of your master PIN. <b style="color:#d98a5f">Anyone with this link gets everything</b> - your chats, your provider keys' usage, same as your PIN. Only hand it to a device you trust, like your own phone.
          </div>
          <button class="small-btn" id="genAccessLinkBtn">generate &amp; copy link</button>
        </div>
      </div>`;

    // ---- Sync & backup (encrypted cross-device sync, pairing, backup file, storage) ----
    html += await SyncUI.html();
    ...
    html += `
      <div class="settings-group" id="appearanceGroup">
        ...
        <div class="settings-card">
          ...
          <div class="settings-row">
            ...
            <div style="display:flex;gap:6px;align-items:center;flex:1">
              ...
              `).join('')}
              <input type="text" id="fontSizeInput" value="${fontSize}" style="max-width:44px;margin-left:4px;font-size:12.5px;padding:5px 6px" />
        ...
        <div class="settings-card">
          <div class="settings-subhead">Personalize</div>
          <div class="settings-row settings-row-top">
            <label>Sidebar</label>
            <div style="flex:1;display:flex;flex-direction:column;gap:8px">
              <div style="display:flex;align-items:center;gap:8px">
                <span class="settings-inline-label">blur</span>
                <input type="range" id="sidebarBlurInput" min="0" max="16" step="1" value="${sidebarBlur}" style="flex:1" />
              </div>
              <div style="display:flex;align-items:center;gap:8px">
                <span class="settings-inline-label">transparency</span>
                <input type="range" id="sidebarOpacityInput" min="0.4" max="1" step="0.05" value="${sidebarOpacity}" style="flex:1" />
              </div>
            </div>
          </div>
          <div class="settings-row">
            ...
            <div style="display:flex;gap:6px">
              ${['sharp', 'rounded', 'pill'].map(v =>
                `<button type="button" class="small-btn bubble-shape-btn ${bubbleShape === v ? 'active' : ''}" data-shape="${v}">${v}</button>`
              ).join('')}
            </div>
          </div>
          <div class="settings-row">
            <label>Bubble fill</label>
            <div style="display:flex;gap:6px">
              ${['solid', 'outline', 'glass'].map(v =>
                `<button type="button" class="small-btn bubble-fill-btn ${bubbleFill === v ? 'active' : ''}" data-fill="${v}">${v}</button>`
              ).join('')}
            </div>
          </div>
          <div class="settings-row">
            <label>Ultra compact</label>
            <div style="display:flex;gap:6px">
              <button type="button" class="small-btn ultra-compact-btn ${ultraCompact ? 'active' : ''}" data-value="1">on</button>
              <button type="button" class="small-btn ultra-compact-btn ${!ultraCompact ? 'active' : ''}" data-value="0">off</button>
            </div>
          </div>
          <div class="settings-row settings-row-top">
            <label>Empty-chat greeting</label>
            <div style="flex:1;display:flex;flex-direction:column;gap:6px">
              <input type="text" id="customGreetingInput" value="${customGreeting.replace(/"/g, '&quot;')}" placeholder="leave blank for the default rotating greetings" style="width:100%" />
              <div class="settings-hint">What you see on a brand-new session, before you've sent anything.</div>
            </div>
          </div>
      </div>`;
    ...
    for (const [id, def] of Object.entries(this.providerDefs)) {
      ...
      html += `<div class="provider-block" data-provider="${id}">
        ...
        ${def.customBaseUrl ? `
        <div style="font-size:10.5px;color:var(--text-muted);margin-bottom:6px">Self-hosted - point this at your own running instance (e.g. a VPS or a Cloudflare Tunnel to a home server), then add its API key below.</div>
        <div class="key-slot">
          <input type="text" placeholder="https://your-instance.example.com/v1" class="baseurl-input" value="${(info.baseUrl || '').replace(/"/g, '&quot;')}" />
          <button class="small-btn" data-action="save-baseurl" data-provider="${id}">save URL</button>
        </div>` : ''}
    ...
    html += `
      <div class="settings-group">
        ...
        <div style="font-size:11px;color:var(--text-muted);margin-bottom:8px">
          Anyone with one of these links can watch that chat live, read-only - no PIN needed, and they never see your other chats, settings or provider keys. New messages appear for them automatically. Revoke a link any time without touching your PIN.
        </div>
        ${shareLinks.length === 0 ? `<div style="font-size:12px;color:var(--text-muted)">No active share links yet.</div>` : shareLinks.map(l => `
          <div class="key-slot">
            <input type="text" value="${SyncUI.esc(l.title)}${l.sessionId ? '' : ' (old snapshot link)'}" disabled />
            <button class="small-btn" data-action="revoke-share" data-token="${SyncUI.esc(l.token)}">revoke</button>
          </div>`).join('')}
      </div>`;
    ...
    try {
      ...
      if (inSync) {
        workerBuildLine = `<span style="color:#7ec98f">Worker in sync</span> <span style="color:var(--text-muted)">(build ${build})</span>`;
      } else if (build && build > EXPECTED_WORKER_BUILD) {
        workerBuildLine = `<span style="color:#d98a5f">Frontend is OUT OF DATE</span> <span style="color:var(--text-muted)">- Worker: ${build}, frontend expects: ${EXPECTED_WORKER_BUILD}. Redeploy index.html/js/css on GitHub Pages, not the Worker.</span>`;
      } else {
        workerBuildLine = `<span style="color:#d98a5f">Worker is OUT OF DATE</span> <span style="color:var(--text-muted)">- live: ${build || 'unknown'}, expected: ${EXPECTED_WORKER_BUILD}. Redeploy worker.js.</span>`;
      }
  ...
  wireEvents() {
    ...
    const sidebarBlurInput = document.getElementById('sidebarBlurInput');
    if (sidebarBlurInput) {
      sidebarBlurInput.addEventListener('input', async (e) => {
        document.documentElement.style.setProperty('--sidebar-blur', e.target.value + 'px');
        await DB.setSetting('sidebarBlur', parseFloat(e.target.value));
      });
    }
    const sidebarOpacityInput = document.getElementById('sidebarOpacityInput');
    if (sidebarOpacityInput) {
      sidebarOpacityInput.addEventListener('input', async (e) => {
        document.documentElement.style.setProperty('--sidebar-opacity', e.target.value);
        await DB.setSetting('sidebarOpacity', parseFloat(e.target.value));
      });
    }
    document.querySelectorAll('.bubble-shape-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        const shape = btn.dataset.shape;
        await DB.setSetting('bubbleShape', shape);
        const appEl = document.getElementById('app');
        appEl.classList.remove('bubble-shape-sharp', 'bubble-shape-rounded', 'bubble-shape-pill');
        appEl.classList.add(`bubble-shape-${shape}`);
        document.querySelectorAll('.bubble-shape-btn').forEach(b => b.classList.toggle('active', b === btn));
      });
    });
    document.querySelectorAll('.bubble-fill-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        const fill = btn.dataset.fill;
        await DB.setSetting('bubbleFill', fill);
        const appEl = document.getElementById('app');
        appEl.classList.remove('bubble-fill-solid', 'bubble-fill-outline', 'bubble-fill-glass');
        appEl.classList.add(`bubble-fill-${fill}`);
        document.querySelectorAll('.bubble-fill-btn').forEach(b => b.classList.toggle('active', b === btn));
      });
    });
    document.querySelectorAll('.ultra-compact-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        const on = btn.dataset.value === '1';
        await DB.setSetting('ultraCompact', on);
        document.getElementById('app').classList.toggle('density-ultra-compact', on);
        document.querySelectorAll('.ultra-compact-btn').forEach(b => b.classList.toggle('active', b === btn));
      });
    });
    const customGreetingInput = document.getElementById('customGreetingInput');
    if (customGreetingInput) {
      let greetingSaveTimer = null;
      customGreetingInput.addEventListener('input', (e) => {
        clearTimeout(greetingSaveTimer);
        const value = e.target.value;
        greetingSaveTimer = setTimeout(() => DB.setSetting('customGreeting', value), 400);
      });
    }
    ...
    if (clearBtn) {
      ...
      clearBtn.addEventListener('click', async () => {
        ...
        const sessions = await DB.listSessions();
        // silent: this clears THIS device only - it must not delete your other devices' copies
        for (const s of sessions) await DB.deleteSession(s.id, { silent: true });
        await Sync.resetLocalState();
    ...
    this.bodyEl.querySelectorAll('[data-action="save-baseurl"]').forEach(btn => {
      btn.addEventListener('click', async () => {
        const block = btn.closest('.provider-block');
        const url = block.querySelector('.baseurl-input').value.trim();
        if (!url) { Toast.show('Enter a base URL first', true); return; }
        try {
          await Providers.setBaseUrl(btn.dataset.provider, url);
          Toast.show('Base URL saved');
          await Settings.render();
        } catch (e) {
          Toast.show('Could not save base URL: ' + e.message, true);
        }
      });
    });
    ...
    this.bodyEl.querySelectorAll('[data-action="refresh-models"]').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        try {
          ...
          listEl.innerHTML = result.models.slice(0, 60).map(m =>
            `<div class="model-pick" data-provider="${btn.dataset.provider}" data-raw="${SyncUI.esc(m)}" style="padding:2px 0;cursor:pointer">${SyncUI.esc(Router.prettifyModelName(m))}${meta[m] && meta[m].free ? ' <span class="viewer-pill ok">free</span>' : ''}</div>`
          ).join('');
          listEl.querySelectorAll('.model-pick').forEach(row => {
            row.addEventListener('click', async () => {
              const modelInput = block.querySelector('.model-input');
              const label = Router.prettifyModelName(row.dataset.raw);
              modelInput.value = label;
              modelInput.dataset.raw = row.dataset.raw;
              await Providers.setModel(row.dataset.provider, row.dataset.raw);
              Toast.show('Model set to ' + label);
              await Settings.render();
            });
          });
        } catch (e) {

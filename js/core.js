...
// ===== logger.js =====
// Event log other modules push into (provider calls, failovers, compiles,
// rejected files, sync, keybind resets, etc) - this is what backs the sidebar
// Console panel. It's a ring buffer of the last 500 entries in memory; the most
// recent 200 are also saved to this device's IndexedDB (never sent anywhere) so
// the Console isn't empty after a reload. Turn that off with `/logs persist off`.
...
const Logger = {
  ...
  // ---- bounded persistence -------------------------------------------------
  PERSIST_MAX: 200,       // entries kept on disk
  persistEnabled: true,
  _persistTimer: null,
  _restored: false,

  // Debounced: at most one IndexedDB write every few seconds, however chatty.
  schedulePersist() {
    if (!this.persistEnabled || !this._restored || this._persistTimer) return;
    this._persistTimer = setTimeout(() => { this._persistTimer = null; this.persistNow(); }, 4000);
  },
  async persistNow() {
    if (!this.persistEnabled || typeof DB === 'undefined') return;
    try {
      const slim = this.buffer.slice(-this.PERSIST_MAX).map(e => ({
        time: e.time, level: e.level, tag: e.tag,
        message: String(e.message).slice(0, 400),
        data: e.data == null ? null : String(typeof e.data === 'string' ? e.data : JSON.stringify(e.data)).slice(0, 300)
      }));
      await DB.setSetting('consoleLog', slim);
    } catch (e) { /* logging must never break the app */ }
  },
  // Called once at startup (after the DB is available).
  async restore() {
    try {
      this.persistEnabled = (await DB.getSetting('consolePersist', true)) !== false;
      if (this.persistEnabled) {
        const saved = await DB.getSetting('consoleLog', []);
        if (Array.isArray(saved) && saved.length) {
          const marker = { time: Date.now(), level: 'info', tag: 'console', message: `restored ${saved.length} entries from your previous session`, data: null };
          this.buffer = [...saved, marker, ...this.buffer].slice(-this.MAX_ENTRIES);
  ...
  async setPersist(on) {
    ...
    await DB.setSetting('consolePersist', !!on);
    if (!on) await DB.deleteSetting('consoleLog');
  },
  ...
  clear() {
    this.buffer = [];
    if (typeof DB !== 'undefined') DB.deleteSetting('consoleLog').catch(() => {});
...
(function applySetupLinkIfPresent() {
  ...
  // A ?pair=XXXX-XXXX link (short-lived, single-use code) just pre-fills the
  // Pair-this-device form - see Sync.init().
  if (pair) {
    try { sessionStorage.setItem('workspace_pending_pair', pair.slice(0, 16)); } catch (e) {}
    window.history.replaceState({}, '', window.location.pathname);
  }
})();

window.addEventListener('DOMContentLoaded', () => {
  if (!localStorage.getItem('workspace_pin') && !localStorage.getItem('workspace_device_token')) {
    setTimeout(() => {
      Toast.show('First time here? Open Settings and enter your PIN - or pair with a code from another device.');
    }, 500);
  }
});
...
const PROVIDER_ICON_SVG = {
  ...
  omniroute: `<path d="M12 2.6 20.2 7.3v9.4L12 21.4 3.8 16.7V7.3z" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linejoin="round"/><circle cx="12" cy="12" r="2.4"/>`
};
...
const PROVIDER_GLYPHS = {
  ...
  ovhcloud: '◈', anthropic: '✳', openai: '◎', deepseek: '◐', omniroute: '⬡'
};

...
// Everything readable is allow-listed by extension - safer than trying to
// blocklist every binary format that could exist. Unknown/binary/executable
// files (.exe, .dll, .bin, .apk, .dmg, archives, etc.) are rejected outright
// before any read is attempted.
const ALLOWED_EXTENSIONS = new Set([
  'pdf', 'docx', 'doc', 'txt', 'md', 'csv', 'json', 'xlsx', 'xls',
  'js', 'ts', 'jsx', 'tsx', 'py', 'html', 'css', 'xml', 'yaml', 'yml',
  'log', 'rtf', 'tsv',
  'png', 'jpg', 'jpeg', 'gif', 'webp', 'svg', 'bmp'
]);
// Belt-and-braces MIME check for the common readable types - extension
// alone can be spoofed/renamed, this catches the obvious mismatches.
...
const FileHandler = {
  // Extension + MIME check - true if this looks like a real, readable file
  // and not a binary/executable/unknown format.
  isReadable(file) {
  ...
  validateBatch(files, existingTotalBytes) {
    ...
    const unreadable = arr.filter(f => !FileHandler.isReadable(f));
    if (unreadable.length) {
      return {
        ok: false,
        message: `Can't attach ${unreadable.map(f => f.name).join(', ')} - only readable documents, text/code files, and images are supported (no executables or unknown binary formats).`
      };
    }

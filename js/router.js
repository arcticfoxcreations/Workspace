...
const Router = {
  ...
  resolveTargets(text, { nicknames, mode, autoProviders }) {
    ...
    if (mode === 'auto') {
      return { targets: Router.guessBestForTask(text), isManualMention: false };
    }
    // manual mode with no mention falls back to whatever's marked active
    return { targets: autoProviders && autoProviders.length ? [autoProviders[0]] : ['gemini'], isManualMention: false };
  },
